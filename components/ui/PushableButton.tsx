...
const PushableButton = ({ text = "Push me" }) => {
  return (
    <button className="relative border-none bg-transparent p-0 cursor-pointer outline-offset-4 transition duration-250 ease-linear">
      ...
      <span className="font-bold tracking-wider relative block px-10 py-3 text-lg text-white rounded-lg bg-primary will-change-transform -translate-y-1 transition-transform duration-600 ease-[cubic-bezier(0.3,0.7,0.4,1)] hover:-translate-y-1.5 active:translate-y-0.5">
        {text}
      </span>
    </button>
  );
